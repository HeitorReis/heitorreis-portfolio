...
export default async function ExperienceSummaryPage() {
  const { experiences, projects, milestones } =
    await getExperienceSummaryData();
    ...
    <DetailPageTemplate
      ...
      sections={[
        ...
        },
        {
          ...
          body:
            milestones.length > 0
              ? "Work that was recognized, presented, led, validated, or applied in professional settings."
              : "No public milestone entries are available yet.",
          list: milestones.map((entry) => entry.title),
        },
      ]}
    />
  );
